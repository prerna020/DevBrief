...
class Issue(BaseModel):
    ...
    # No default: model output must explicitly provide an exact rule or null.
    matched_rule: str | None = Field(alias="matchedRule")
...
def _require_all_object_properties(value: object) -> object:
    ...
    if isinstance(result.get("properties"), dict):
        result["required"] = list(result["properties"].keys())  # type: ignore[index]
    return result
